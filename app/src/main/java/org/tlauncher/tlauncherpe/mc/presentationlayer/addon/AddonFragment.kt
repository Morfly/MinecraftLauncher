...
class AddonFragment : BaseFragment<AddonContract.Presenter, FragmentAddonBinding>(), AddonContract.View,
    ...
    var listData : MutableList<AddonItemViewModel> = ArrayList()
    ...
    override fun setListData(list : ArrayList<AddonItemViewModel>, refresh : Boolean) {
        ...
        if (refresh) {
            ...
            //isShow need for show My addons vews
    ...
    override fun getLayoutId() : Int = R.layout.fragment_addon
    ...
    override fun onViewCreated(view: View?, savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState?.getInt("firstSort") != null) {
            ...
            saveAddonFAB(firstSort,activity)
        }
        //get screen stage
    ...
    override fun onCreateView(savedInstanceState : Bundle?) {
        ...
        adapter = AddonAdapter(ArrayList<AddonItemViewModel>(), this, presenter, activity, activity)
        ...
        val firstAd: ArrayAdapter<CharSequence>? = ArrayAdapter.createFromResource(activity,
                R.array.firstSortAddons, android.R.layout.simple_spinner_item)
            ...
            override fun onItemSelected(parent : AdapterView<*>?, view : View?, position : Int, id : Long) {
                when (position) {
                    0 -> {
                        ...
                        (activity as? MainActivity)?.hideShowFabButton("addon")
    ...
    override fun onOptionsItemSelected(item : MenuItem?) : Boolean {
        if (item?.itemId == R.id.highlight_all) {
            ...
            array = AddonJsonCreator(listData as java.util.ArrayList<AddonItemViewModel>)
            backupList()
        }
    ...
    fun backupList(){
        ...
        val f = File(Environment.getExternalStorageDirectory().toString() +
                context.resources.getString(R.string.backup_addons))
        if (f.exists()) {
        ...
        searchView.setOnQueryTextListener(object : SearchView.OnQueryTextListener {
            ...
            override fun onQueryTextChange(newText : String?) : Boolean {
                if (newText.isNullOrEmpty().not()) {
                    ...
                    var dat : MutableList<AddonItemViewModel> = ArrayList()
                    listData.forEachIndexed { _, addonItemViewModel ->
                        var txt : String = newText!!
                        if (addonItemViewModel.name!!.contains(txt, true)) {
                            dat.add(addonItemViewModel)
                        }
                    }
                    adapter?.updateItem(dat as ArrayList<AddonItemViewModel>)
                } else {
                    adapter?.updateItem(listData as ArrayList<AddonItemViewModel>)
                }
                return false
            }
        })
        ...
        if (requestCode == DETAILS_REQUEST_CODE && resultCode == Activity.RESULT_OK) {
            ...
            if (data != null) {
                ...
                unz(path!!, Environment.getExternalStorageDirectory().toString()
                        + context.resources.getString(R.string.addons_folder), i)
                        .subscribe({
                            myAddons.name = it.name
                            myAddons.id = it.id
                            myAddons.filePath = it.path
                            presenter.saveToMy(myAddons, 1)
                            Log.e("unzip", "success")
                        }, { Log.e("unzip", "error") })
                presenter.saveToMy(myAddons, 1)
                Log.e("unzip", "success")
            ...
            if (data != null) {
                ...
                unzipM(path!!, Environment.getExternalStorageDirectory().toString()
                        + context.resources.getString(R.string.addons_folder))
                        ...
                        .subscribe({
                            ...
                            (0..json.length() - 1)
                                    .map { Gson().fromJson(Gson().fromJson(json.getJSONObject(it).toString(), JsonElement::class.java), MyAddons::class.java) }
