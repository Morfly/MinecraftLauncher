...
import com.github.nuptboyzhb.lib.SuperSwipeRefreshLayout.OnPullRefreshListener
...
import org.tlauncher.tlauncherpe.mc.databinding.FragmentWorldBinding
...
import org.tlauncher.tlauncherpe.mc.presentationlayer.world.adapter.WorldAdapter
import java.io.File
import java.text.SimpleDateFormat
import java.util.*


class WorldFragment : BaseFragment<WorldContract.Presenter, FragmentWorldBinding>(), WorldContract.View,
        View.OnClickListener, BottomNavigationView.OnNavigationItemSelectedListener {

    var options : MenuItem? = null
    var adapter : WorldAdapter? = null
    var listData : MutableList<WorldItemViewModel> = ArrayList()
    ...
    override fun setListData(list : ArrayList<WorldItemViewModel>, refresh : Boolean) {
        ...
        if (refresh) {
            ...
            //isShow need for show My worlds vews
    ...
    override fun onViewCreated(view: View?, savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState?.getInt("firstSort") != null) {
            ...
            saveWorldFAB(firstSort,activity)
        }
        //get screen stage
    ...
    override fun onCreateView(savedInstanceState : Bundle?) {
        ...
        adapter = WorldAdapter(ArrayList<WorldItemViewModel>(), this, presenter, activity, activity)
        ...
        val firstAd: ArrayAdapter<CharSequence>? = ArrayAdapter.createFromResource(activity,
                R.array.firstSortMods, android.R.layout.simple_spinner_item)
            ...
            override fun onItemSelected(parent : AdapterView<*>?, view : View?, position : Int, id : Long) {
                when (position) {
                    0 -> {
                        ...
                        (activity as? MainActivity)?.hideShowFabButton("world")
        ...
        binding.swipeRefresh.setOnPullRefreshListener(object : OnPullRefreshListener {
    ...
    override fun onOptionsItemSelected(item : MenuItem?) : Boolean {
        if (item?.itemId == R.id.highlight_all) {
            ...
            listData.forEachIndexed { index, addonItemViewModel ->
                chackMap.put(addonItemViewModel.id,addonItemViewModel.filePath!!)
            }
            array = WorldJsonCreator(listData as ArrayList<WorldItemViewModel>)
            backupList()
        }
    ...
    fun backupList(){
        ...
        val f = File(Environment.getExternalStorageDirectory().toString() +
                context.resources.getString(R.string.backup_maps))
        if (f.exists()) {
        ...
        searchView.setOnQueryTextListener(object : SearchView.OnQueryTextListener {
            ...
            override fun onQueryTextChange(newText : String?) : Boolean {
                if (newText.isNullOrEmpty().not()) {
                    ...
                    var dat : MutableList<WorldItemViewModel> = ArrayList()
                    listData.forEachIndexed { _, worldItemViewModel ->
                        var txt : String = newText!!
                        if (worldItemViewModel.name!!.contains(txt, true)) {
                            dat.add(worldItemViewModel)
                        }
                    }
                    adapter?.updateItem(dat as ArrayList<WorldItemViewModel>)
                } else {
                    adapter?.updateItem(listData as ArrayList<WorldItemViewModel>)
                }
                return false
            }
        })
        ...
        if (requestCode == MainActivity.DETAILS_REQUEST_CODE && resultCode == Activity.RESULT_OK) {
            ...
            if (data != null) {
                ...
                unz(path!!, Environment.getExternalStorageDirectory().toString()
                        + context.resources.getString(R.string.maps_folder), i)
                        .subscribe({
                            myMods.name = it.name
                            myMods.id = it.id
                            myMods.filePath = it.path
                            presenter.saveToMy(myMods, 1)
                            Log.e("unzip", "success")
                        }, { Log.e("unzip", "error") })
                presenter.saveToMy(myMods, 1)
                Log.e("unzip", "success")
            ...
            if (data != null) {
                ...
                unzipM(path!!, Environment.getExternalStorageDirectory().toString()
                        + context.resources.getString(R.string.maps_folder))
                        ...
                        .subscribe({
                            ...
                            (0..json.length() - 1)
                                    .map { Gson().fromJson(Gson().fromJson(json.getJSONObject(it).toString(), JsonElement::class.java), MyMods::class.java) }
