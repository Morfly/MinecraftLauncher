...
class TextureFragment : BaseFragment<TextureContract.Presenter, FragmentTextureBinding>(), TextureContract.View,
        View.OnClickListener, BottomNavigationView.OnNavigationItemSelectedListener {

    var options : MenuItem? = null
    var adapter : TextureAdapter? = null
    var listData : MutableList<TextureItemViewModel> = ArrayList()
    ...
    override fun setListData(list : ArrayList<TextureItemViewModel>, refresh : Boolean) {
        ...
        if (refresh) {
            ...
            //isShow need for show My texturs vews
    ...
    override fun getLayoutId() : Int = R.layout.fragment_texture
    ...
    override fun onViewCreated(view: View?, savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState?.getInt("firstSort") != null) {
            ...
            saveTextureFAB(firstSort,activity)
        }
        //get screen stage
    ...
    override fun onCreateView(savedInstanceState : Bundle?) {
        ...
        adapter = TextureAdapter(ArrayList<TextureItemViewModel>(), this, presenter, activity, activity)
        ...
        val firstAd: ArrayAdapter<CharSequence>? = ArrayAdapter.createFromResource(activity,
                R.array.firstSortTextures, android.R.layout.simple_spinner_item)
            ...
            override fun onItemSelected(parent : AdapterView<*>?, view : View?, position : Int, id : Long) {
                when (position) {
                    0 -> {
                        ...
                        (activity as? MainActivity)?.hideShowFabButton("texture")
    ...
    override fun onOptionsItemSelected(item : MenuItem?) : Boolean {
        if (item?.itemId == R.id.highlight_all) {
            ...
            listData.forEachIndexed { index, textureItemViewModel ->
                chackMap.put(textureItemViewModel.id,textureItemViewModel.filePath!!)
            }
            array = TextureJsonCreator(listData as ArrayList<TextureItemViewModel>)
            backupList()
        }
    ...
    fun backupList(){
        ...
        val f = File(Environment.getExternalStorageDirectory().toString() +
                context.resources.getString(R.string.backup_textures))
        if (f.exists()) {
        ...
        myIntent.putExtra("type","texture")
        ...
        searchView.setOnQueryTextListener(object : SearchView.OnQueryTextListener {
            ...
            override fun onQueryTextChange(newText : String?) : Boolean {
                if (newText.isNullOrEmpty().not()) {
                    ...
                    var dat : MutableList<TextureItemViewModel> = ArrayList()
                    listData.forEachIndexed { _, textureItemViewModel ->
                        var txt : String = newText!!
                        if (textureItemViewModel.name!!.contains(txt, true)) {
                            dat.add(textureItemViewModel)
                        }
                    }
                    adapter?.updateItem(dat as ArrayList<TextureItemViewModel>)
                } else {
                    adapter?.updateItem(listData as ArrayList<TextureItemViewModel>)
                }
                return false
            }
        })
        ...
        if (requestCode == DETAILS_REQUEST_CODE && resultCode == Activity.RESULT_OK) {
            ...
            if (data != null) {
                ...
                unz(path!!, Environment.getExternalStorageDirectory().toString()
                        + context.resources.getString(R.string.textures_folder), i)
                        .subscribe({
                            myTextures.name = it.name
                            myTextures.id = it.id
                            myTextures.filePath = it.path
                            presenter.saveToMy(myTextures, 1)
                            Log.e("unzip", "success")
                        }, { Log.e("unzip", "error") })
                presenter.saveToMy(myTextures, 1)
                Log.e("unzip", "success")
            ...
            if (data != null) {
                ...
                unzipM(path!!, Environment.getExternalStorageDirectory().toString()
                        + context.resources.getString(R.string.textures_folder))
                        ...
                        .subscribe({
                            ...
                            (0..json.length() - 1)
                                    .map { Gson().fromJson(Gson().fromJson(json.getJSONObject(it).toString(), JsonElement::class.java), MyTextures::class.java) }
    ...
    override fun updateList(id: Int, secondPos: Int) {
        adapter?.updateItem(id, secondPos)
    }
}