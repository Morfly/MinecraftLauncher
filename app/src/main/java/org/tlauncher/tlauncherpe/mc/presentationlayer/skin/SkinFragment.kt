...
import org.tlauncher.tlauncherpe.mc.databinding.FragmentSkinBinding
...
import org.tlauncher.tlauncherpe.mc.presentationlayer.skin.adapter.SkinAdapter
import java.io.File
import java.text.SimpleDateFormat
import java.util.*

class SkinFragment : BaseFragment<SkinContract.Presenter, FragmentSkinBinding>(), SkinContract.View,
    ...
    var listData : MutableList<SkinItemViewModel> = ArrayList()
    ...
    override fun setListData(list : ArrayList<SkinItemViewModel>, refresh : Boolean) {
        ...
        if (refresh) {
            ...
            //isShow need for show My skins vews
    ...
    override fun onViewCreated(view: View?, savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState?.getInt("firstSort") != null) {
            ...
            saveSkinFAB(firstSort,activity)
        }
        //get screen stage
    ...
    override fun onCreateView(savedInstanceState : Bundle?) {
        ...
        adapter = SkinAdapter(ArrayList<SkinItemViewModel>(), this, presenter, activity, activity)
        ...
        val firstAd: ArrayAdapter<CharSequence>? = ArrayAdapter.createFromResource(activity,
                R.array.firstSortSkins, android.R.layout.simple_spinner_item)
            ...
            override fun onItemSelected(parent : AdapterView<*>?, view : View?, position : Int, id : Long) {
                when (position) {
                    0 -> {
                        ...
                        (activity as? MainActivity)?.hideShowFabButton("skin")
    ...
    override fun onOptionsItemSelected(item : MenuItem?) : Boolean {
        if (item?.itemId == R.id.highlight_all) {
            ...
            listData.forEachIndexed { index, skinItemViewModel ->
                chackMap.put(skinItemViewModel.id,skinItemViewModel.filePath!!)
            }
            array = SkinJsonCreator(listData as ArrayList<SkinItemViewModel>)
            backupList()
        }
    ...
    fun backupList(){
        ...
        val f = File(Environment.getExternalStorageDirectory().toString() +
                context.resources.getString(R.string.backup_skins))
        if (f.exists()) {
        ...
        searchView.setOnQueryTextListener(object : SearchView.OnQueryTextListener {
            ...
            override fun onQueryTextChange(newText : String?) : Boolean {
                if (newText.isNullOrEmpty().not()) {
                    ...
                    var dat : MutableList<SkinItemViewModel> = ArrayList()
                    listData.forEachIndexed { _, skinItemViewModel ->
                        var txt : String = newText!!
                        if (skinItemViewModel.name!!.contains(txt, true)) {
                            dat.add(skinItemViewModel)
                        }
                    }
                    adapter?.updateItem(dat as ArrayList<SkinItemViewModel>)
                } else {
                    adapter?.updateItem(listData as ArrayList<SkinItemViewModel>)
                }
                return false
            }
        })
        ...
        if (requestCode == MainActivity.DETAILS_REQUEST_CODE && resultCode == Activity.RESULT_OK) {
            ...
            if (data != null) {
                ...
                unz(path!!, Environment.getExternalStorageDirectory().toString()
                        + context.resources.getString(R.string.skins_folder), i)
                        .subscribe({
                            mySkins.name = it.name
                            mySkins.id = it.id
                            mySkins.filePath = it.path
                            presenter.saveToMy(mySkins, 1)
                            Log.e("unzip", "success")
                        }, { Log.e("unzip", "error") })
                presenter.saveToMy(mySkins, 1)
                Log.e("unzip", "success")
            ...
            if (data != null) {
                ...
                unzipM(path!!, Environment.getExternalStorageDirectory().toString()
                        + context.resources.getString(R.string.skins_folder))
                        ...
                        .subscribe({
                            ...
                            (0..json.length() - 1)
                                    .map { Gson().fromJson(Gson().fromJson(json.getJSONObject(it).toString(), JsonElement::class.java), MySkins::class.java) }
    ...
    override fun updateList(pos: Int, secondPos: Int) {
        adapter?.updateItem(pos, secondPos)
    }
}